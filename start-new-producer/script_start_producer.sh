...
## cmd
./nodeos --max-irreversible-block-age -1 --contracts-console --genesis-json ./genesis.json --blocks-dir ./blocks --config-dir ./ --data-dir ./ --http-server-address $httpServerAddr --p2p-listen-endpoint $p2pListenEndpoint --max-clients 14 --p2p-max-nodes-per-host 14 --enable-stale-production --producer-name $producerName --private-key $producerKeyPair $producerPeerList $producerPluginList --http-validate-host false --delete-all-blocks  2>>./stderr &
